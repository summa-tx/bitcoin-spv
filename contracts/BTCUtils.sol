...
library BTCUtils {
    ...
    function extractPrefix(bytes memory _b) internal pure returns (bytes memory) {
    ...
    function extractSequenceLE(bytes memory _b) internal pure returns (bytes memory) {return _b.slice(37, 4);}

    /// @notice          Extracts the sequence from the input in a tx
    /// @dev             Sequence is a 4-byte little-endian number
    /// @param _b        The input
    /// @return          The sequence number (big-endian uint)
    function extractSequence(bytes memory _b) internal pure returns (uint32) {
    ...
    function extractOutpoint(bytes memory _b) internal pure returns (bytes memory) {return _b.slice(0, 36);}

    /// @notice          Extracts the tx input tx id from the input in a tx
    /// @dev             32 byte tx id
    /// @param _b        The input
    /// @return          The tx id (little-endian bytes)
    function extractTxIdLE(bytes memory _b) internal pure returns (bytes32) {return _b.slice(0, 32).toBytes32();}

    /// @notice          Extracts the tx input tx id from the input in a tx
    /// @dev             32 byte tx id
    /// @param _b        The input
    /// @return          The tx id (big-endian bytes)
    function extractTxId(bytes memory _b) internal pure returns (bytes32) {
    ...
    function extractTxIndexLE(bytes memory _b) internal pure returns (bytes memory) {return _b.slice(32, 4);}

    /// @notice          Extracts the tx input index from the input in a tx
    /// @dev             4 byte tx index
    /// @param _b        The input
    /// @return          The tx index (big-endian uint)
    function extractTxIndex(bytes memory _b) internal pure returns (uint32) {
    ...
    function extractOutputScriptLen(bytes memory _b) internal pure returns (bytes memory) {return _b.slice(8, 1);}

    /// @notice          Extracts the value bytes from the output in a tx
    /// @dev             Value is an 8-byte little-endian number
    /// @param _b        The tx
    /// @return          The output value as LE bytes
    function extractValueLE(bytes memory _b) internal pure returns (bytes memory) {return _b.slice(0, 8);}

    /// @notice          Extracts the value from the output in a tx
    /// @dev             Value is an 8-byte little-endian number
    /// @param _b        The tx
    /// @return          The output value
    function extractValue(bytes memory _b) internal pure returns (uint64) {
    ...
    function extractHash(bytes memory _b) internal pure returns (bytes memory) {
    ...
    function extractLocktimeLE(bytes memory _b) internal pure returns (bytes memory) {return lastBytes(_b, 4);}
    ...
    function extractLocktime(bytes memory _b) internal pure returns (uint32) {
    ...
    function extractNumInputsBytes(bytes memory _b) internal pure returns (bytes memory) {
        return _b.slice(6, 1);
    }
    ...
    /// @param _b        The tx to evaluate
    /// @return          The number of inputs
    function extractNumInputs(bytes memory _b) internal pure returns (uint8) {
        uint256 _n = bytesToUint(extractNumInputsBytes(_b));
        require(_n < 0xfd, "Multi-byte VarInts not supported");  // Error on VarInts
        return uint8(_n);
    }
    ...
    function findNumOutputs(bytes memory _b) internal pure returns (uint256) {
    ...
    function extractNumOutputsBytes(bytes memory _b) internal pure returns (bytes memory) {
    ...
    /// @param _b        The tx to evaluate
    /// @return          The number of outputs
    function extractNumOutputs(bytes memory _b) internal pure returns (uint8) {
        uint256 _offset = findNumOutputs(_b);
    ...
    function extractInputAtIndex(bytes memory _b, uint8 _index) internal pure returns (bytes memory) {
    ...
    function determineOutputLength(bytes memory _b) internal pure returns (uint256) {
    ...
    function extractOutputAtIndex(bytes memory _b, uint8 _index) internal pure returns (bytes memory) {
    ...
    function extractMerkleRootLE(bytes memory _b) internal pure returns (bytes memory) {return _b.slice(36, 32);}

    /// @notice          Extracts the transaction merkle root from a block header
    /// @dev             Use verifyHash256Merkle to verify proofs with this root
    /// @param _b        The header
    /// @return          The merkle root (big-endian)
    function extractMerkleRootBE(bytes memory _b) internal pure returns (bytes memory) {
    ...
    function extractTarget(bytes memory _b) internal pure returns (uint256) {
    ...
    function extractPrevBlockLE(bytes memory _b) internal pure returns (bytes memory) {return _b.slice(4, 32);}

    /// @notice          Extracts the previous block"s hash from a block header
    /// @dev             Block headers do NOT include block number :(
    /// @param _b        The header
    /// @return          The previous block"s hash (big-endian)
    function extractPrevBlockBE(bytes memory _b) internal pure returns (bytes memory) {
    ...
    function extractTimestampLE(bytes memory _b) internal pure returns (bytes memory) {return _b.slice(68, 4);}

    /// @notice          Extracts the timestamp from a block header
    /// @dev             Time is not 100% reliable
    /// @param _b        The header
    /// @return          The timestamp (uint)
    function extractTimestamp(bytes memory _b) internal pure returns (uint32) {
    ...
    function verifyHash256Merkle(bytes memory _a, uint _index) internal pure returns (bool) {
