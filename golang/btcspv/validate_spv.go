...
func ValidateHeaderChain(headers []byte) (sdk.Uint, error) {
	...
	if len(headers)%80 != 0 {
		return sdk.ZeroUint(), errors.New("2")
	}
	...
	for i := 0; i < len(headers)/80; i++ {
		...
		if i != 0 {
			if !ValidateHeaderPrevHash(header, digest) {
				// TODO: Is there a better way to return error code in go?
				return sdk.ZeroUint(), errors.New("1")
			}
		}
		...
		if !ValidateHeaderWork(digest, target) {
			return sdk.ZeroUint(), errors.New("3")
		}
...
func (s SPVProof) Validate() (bool, error) {
	...
	if !validVin {
		return false, errors.New("11")
	}
	validVout := ValidateVout(s.Vout)
	if !validVout {
		return false, errors.New("12")
	}
